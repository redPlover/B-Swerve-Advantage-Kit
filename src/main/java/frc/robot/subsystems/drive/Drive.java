      ...
      switch (driveMode) {
        case NORMAL:
          ...
          SwerveDriveKinematics.desaturateWheelSpeeds(setpointStates,
              ...
              Logger.getInstance().recordOutput("SwerveModuleStates/debug", setpointStates);
          ...
          for (int i = 0; i < 4; i++) {
            ...
            if (isStationary) {
              ...
              moduleIOs[i].setTurnVoltage(
                  turnFeedback[i].calculate(turnPositions[i].getRadians(),
                      setpointStates[i].angle.getRadians()));
            }
            ...
            double velocityRadPerSec =
                setpointStates[i].speedMetersPerSecond / wheelRadius;
